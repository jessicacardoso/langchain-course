...
# vectorstore = Chroma(persist_directory="langchain_docs", embedding_function=embeddings)
vectorstore = Milvus(
    collection_name=os.getenv("INDEX_NAME", "langchain_docs"),
    connection_args={
...
async def main():
    ...
    for tavily_crawl_result_item in res["results"]:
        ...
        if not tavily_crawl_result_item.get("raw_content"):
            log_warning(
                f"TavilyCrawl: No content extracted from {tavily_crawl_result_item['url']}, skipping..."
            )
            continue
